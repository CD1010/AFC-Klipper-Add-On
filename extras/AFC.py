...
class afc:
    ...
    def handle_connect(self):
        ...
        self.gcode.register_command('AFC_SET_EXTRUDER_SENSOR', self.cmd_AFC_SET_EXTRUDER_SENSOR, desc='Manually set the state of a toolhead sensor')
    ...
    def cmd_AFC_SET_EXTRUDER_SENSOR(self, gcmd):
        ...
        if extruder_name not in self.tools:
            raise gcmd.error("Unknown extruder '{}'".format(extruder_name))
        
        extruder_obj = self.tools[extruder_name]

        if sensor_name.lower() == 'start':
            extruder_obj.tool_start_state = bool(state)
            self.logger.info("Set {} start sensor state to {}".format(extruder_name, bool(state)))
        elif sensor_name.lower() == 'end':
            extruder_obj.tool_end_state = bool(state)
            self.logger.info("Set {} end sensor state to {}".format(extruder_name, bool(state)))
        else:
            raise gcmd.error("Invalid sensor name '{}'. Use 'start' or 'end'.".format(sensor_name))
